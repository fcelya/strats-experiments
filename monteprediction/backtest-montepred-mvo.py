...
import os
from mvo import MVOPortfolio

class Backtest:
    def __init__(self, asset_universe, start_date, end_date, initial_wealth, transaction_cost_type, transaction_cost, bounds):
        """
        ...
        self.prices, self.predictions = self.load_price_data()
    ...
    def _verify_rebalance(self,current_weights,target_weights,threshold=0.05):
    ...
    def calculate_weights(self, available_data):
        ...
        mvo_portfolio = MVOPortfolio(mean, cov,self.bounds[0],self.bounds[1])
        asset_weights = mvo_portfolio.optimize_portfolio()
        target_weights = np.append(asset_weights,cash_weight)
...
if __name__=='__main__':
    ...
    backtest = Backtest(asset_universe, start_date, end_date, initial_wealth, transaction_cost_type, transaction_cost, (0,0.3))
    ...
    df.to_csv(os.path.join(os.path.dirname(__file__),'results','montepred_mvo.csv'))
